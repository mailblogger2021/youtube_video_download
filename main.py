...
def get_short_links(channel_url, progress_var, progress_label_var):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if "entries" in result:
            ...
            short_links = [
                f"https://www.youtube.com/shorts/{video_id}" for video_id in video_ids
            ]
            return short_links
        else:
    ...
    for index, link in enumerate(links, start=1):
        ...
        try:
            ...
            progress_label_var.set(
                f"Downloading video {index}/{total_links}: {link} - Downloaded successfully"
            )
        except subprocess.CalledProcessError as e:
            print(f"Error downloading {link}: {e}")
            progress_label_var.set(
                f"Downloading video {index}/{total_links}: {link} - Failed to download: {e}"
            )
        finally:
...
def on_start_button_click(folder_var, channel_entry, progress_var, progress_label_var):
    ...
    Thread(
        target=download_videos_from_links,
        args=(short_links, output_directory, progress_var, progress_label_var),
    ).start()
...
folder_entry = ttk.Entry(
    main_frame, textvariable=folder_var, state="readonly", width=50
)
...
start_button = ttk.Button(
    ...
    command=lambda: on_start_button_click(
        folder_var, channel_entry, progress_var, progress_label_var
    ),
)
